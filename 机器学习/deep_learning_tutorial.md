...
在讲这个算法之前说一下移动指数加权平均。移动指数加权平均法加权就是根据同一个移动段内不同时间的数据对预测值的影响程度，分别给予不同的权数，然后再进行平均移动以预测未来值。假定给定一系列数据值$$x_{1}, x_{2},x_{3},……x_{n}$$。那么，我们根据这些数据来拟合一条曲线，所得的值$$v_{1}, v_{2}…..$$就是如下的公式：
$$
v_{0} = 0\\
v_{1} = \beta v_{0}+(1-\beta )x_{0} \\
v_{2} = \beta v_{1}+(1-\beta )x_{1} \\
$$
参数更新公式为：  
$$
